...
**Keyingi qadam (tdesktop sessiyasi):** A4+ (startdagi 727 ms blok: v17 qamrovchi indeks `4a477c8abd`, `RestoreDeletedChats` DB so'rovi va `PruneStaleActivityHistory` fonga `c27bdccb60`), A5 (o'lchov kodi `CUSTOMMOD_PROFILE=1` ortiga olindi, issiq yo'llar tozalandi `6eb4b30075`), hamda A6 (B1: inject qilingan o'chirilgan xabarlarda readInboxTill xatosi to'xtatildi va unread badge xavfsizlandi `837ce8f234`; B2: legacy account_id=0 tozalash skripti tayyorlandi va nusxada sinab ko'rildi `ee4b9ec9dc`; B3: ActivityCacheLoad 132 ms o'lchandi) kodlari tayyor -> **qo'lda build (~55 daq)** -> start logini tekshirish (727 ms blok, profayler jadvallari va readInboxTill xatolari yo'qligi) -> A7 reliz (avval VPS mirror tekshiruvi). A2 tasdig'i va A3 (sync uchun alohida ulanish) server deploy bosqichiga ko'chdi. Batafsil:
...
| 0 | 🔴 **A20 — lahza-nuqtalar davrlar ro'yxatiga tushmaydi** 🆕 | A19 BILAN BIRGA qilinadi. Story/qo'lda nuqtalar `online:`+`offline:` juftligi bo'lmagani uchun tashlab yuboriladi. Sxema o'zgarmaydi. [`specs/2026-08-31-a20-instant-activity-points.md`](specs/2026-08-31-a20-instant-activity-points.md) |
